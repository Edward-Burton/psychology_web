...
public class GoodServiceImpl implements GoodService {
	...
	public Page<PsychoGood> queryGoodList(Map<String, Object> paramMap) {
		Page<PsychoGood> page = new Page<PsychoGood>((Integer) paramMap.get("pageno"),
	...
	public String notify(Map<String, String> conversionParams) {
		...
		try {
			// 调用SDK验证签名
			signVerified = AlipaySignature.rsaCheckV1(conversionParams, PropertiesConfig.getKey("alipay_public_key"),
		...
		if (signVerified) {
			...
			if (alipaymentOrder != null && totalAmount.equals(String.valueOf(alipaymentOrder.getOrder_total_amount()))
					&& PropertiesConfig.getKey("app_id").equals(appId)) {
	...
	public int checkAlipay(String outTradeNo) {
		...
		try {
			...
			AlipayClient alipayClient = new DefaultAlipayClient(PropertiesConfig.getKey("gatewayUrl"),
					PropertiesConfig.getKey("app_id"), PropertiesConfig.getKey("merchant_private_key"),
					PropertiesConfig.getKey("format"), PropertiesConfig.getKey("charset"),
