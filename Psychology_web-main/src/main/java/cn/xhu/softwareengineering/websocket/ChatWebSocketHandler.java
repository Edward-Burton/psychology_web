package cn.xhu.softwareengineering.websocket;
...
import cn.xhu.softwareengineering.util.GsonUtils;
...
public class ChatWebSocketHandler implements WebSocketHandler {
	...
	private static final List<Message> messageList;
	...
	public void afterConnectionEstablished(WebSocketSession webSocketSession) throws Exception {
		...
		PsychoUser loginUser=(PsychoUser) webSocketSession.getAttributes().get("loginUser");
		System.out.println("loginUser:"+loginUser.getPsychouser_id());
		USER_SOCKETSESSION_MAP.put(loginUser.getPsychouser_id(), webSocketSession);
		System.out.println(USER_SOCKETSESSION_MAP);
	}
	...
	public void handleTransportError(WebSocketSession webSocketSession, Throwable exception){
		...
		for (Entry<Integer, WebSocketSession> entry : entrySet) {
			if(entry.getKey().equals(loginUser.getPsychouser_id())){
				msg.setText("【"+loginUser.getPsychouser_name()+"】已经退出。。。！");
				//清除在线会话
		...
		//并查找出在线用户的WebSocketSession（会话），将其移除（不再对其发消息了。。）
		for (Entry<Integer, WebSocketSession> entry : entrySet) {
			msg.getUserList().add((PsychoUser)entry.getValue().getAttributes().get("loginUser"));
		}
		saveMessage();
		TextMessage message = new TextMessage(GsonUtils.toJson(msg));
		try {
			sendMessageToUser(msg.getTo(),message);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void afterConnectionClosed(WebSocketSession webSocketSession, CloseStatus closeStatus) throws Exception {
		...
		for (Entry<Integer, WebSocketSession> entry : entrySet) {
			if(entry.getKey().equals(loginUser.getPsychouser_id())){
				//群发消息告知大家
				msg.setText("【"+loginUser.getPsychouser_name()+"】已下线");
				//清除在线会话
				USER_SOCKETSESSION_MAP.remove(entry.getKey());
				//记录日志：
				System.out.println("Socket会话已经移除:用户ID" + entry.getKey());
	...
	private void saveMessage() {
		DBaseUtil.insertMessageDate(messageList, DBaseUtil.getConn(AreaBaseUtil.getBaseProperties("classpath:config/jdbc.properties")));
		messageList.clear();
	}
